...
bool t_fs::does_exist(const t_path& path) const {
    return std::filesystem::exists(path);
}

bool t_fs::is_directory(const t_path& path) const {
    return std::filesystem::is_directory(path);
}

void t_fs::do_create_directories(const t_path& path) const {
    if (does_exist(path)) {
        return;
    }
...
std::set<t_path> t_fs::get_paths_in_directory(const t_path& root) const {
    using t_entry = std::filesystem::directory_entry;
    std::set<t_path> paths;
...
t_size t_fs::get_size(const t_path& path) const {
    return std::filesystem::file_size(path);
}

t_timestamp t_fs::get_timestamp(const t_path& path) const {
    return std::filesystem::last_write_time(path);
}

void t_fs::do_write_to_drive(const t_qt_image& image, const t_path& path) const {
...
t_qt_image t_fs::do_read_from_drive(const t_path& path) const {
    t_qt_image image {};
