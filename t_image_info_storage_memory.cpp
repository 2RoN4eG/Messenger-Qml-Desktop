...
t_image_info_extended::t_image_info_extended(const t_image_id avatar_id, const t_peer peer, t_image_info&& avatar_info)
    : _image_id { avatar_id }
    , _peer { peer }
...
t_image_info t_image_info_storage_memory::get_image_info(const t_image_id image_id, const i_path_holder& path_holder) const {
    ...
    if (t_image_infos_iterator it = std::find_if(_image_infos.begin(), _image_infos.end(), predicate); it != _image_infos.end()) {
        ...
        image_info._path = path_holder(it->_peer, filename);
...
void t_image_info_storage_memory::set_image_info(const t_image_id image_id, const t_peer peer, const t_url& url, const t_thumb_hash& thumb_hash) {
    _image_infos.emplace(image_id, peer, t_image_info { url, thumb_hash });
}
