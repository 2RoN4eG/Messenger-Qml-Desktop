...
{
    const t_fs_path& image_path = _image_component_storage.get_image_fs_path(_path_maker, image_id);
    ...
    const t_url& image_url = _image_component_storage.get_image_url(image_id);
    run_async_image_downloading(image_id, image_url);

    const t_thumb_hash& image_thumb_hash = _image_component_storage.get_image_thumb_hash(image_id);
...
void t_qt_image_response::on_image_downloaded(const t_image_id image_id)
{
    ...
    _image_storage.write_image_to_drive(image_path, _image);
