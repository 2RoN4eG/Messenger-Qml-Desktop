...
public:
    ...
    virtual t_path make_path_for_peer(const t_peer_id peer) const = 0;
};

class i_avatar_path_maker {
public:
    virtual t_path make_path_for_avatar(const t_peer_id peer) const = 0;
    
    virtual t_path make_path_for_avatar(const t_peer_id peer, const t_avatar_type avatar_type) const = 0;
    
    virtual t_path make_path_for_avatar(const t_peer_id peer, const t_avatar_type avatar_type, const t_filename& filename) const = 0;
};

class i_photo_path_maker {
public:
    virtual t_path make_path_for_photo(const t_peer_id peer) const = 0;
    
    virtual t_path make_path_for_photo(const t_peer_id peer, const t_filename& filename) const = 0;
};
...
{
public:
    t_path_maker(const t_path& root, const t_peer_id& self);

    // (i_self_and_peer_maker's implementation)

    t_path make_path_for_self() const override;
    
    t_path make_path_for_peer(const t_peer_id peer) const override;
    ...
    t_path make_path_for_avatar(const t_peer_id peer) const override;
    
    t_path make_path_for_avatar(const t_peer_id peer, const t_avatar_type avatar_type) const override;
    
    t_path make_path_for_avatar(const t_peer_id peer, const t_avatar_type avatar_type, const t_filename& filename) const override;

    // photo section (i_photo_path_maker's implementation)

    // /root/self_peer/peer/photos/{list-of-photos-or-single-photo}
    
    t_path make_path_for_photo(const t_peer_id peer) const override;
    
    t_path make_path_for_photo(const t_peer_id peer, const t_filename& filename) const override;
...
public:
    ...
    t_path operator()(const t_peer_id peer, const t_filename& filename) const override { return _image_path_maker.make_path_for_avatar(peer, _avatar_type, filename); }
...
public:
    ...
    t_path operator()(const t_peer_id peer, const t_filename& filename) const override { return _photo_path_maker.make_path_for_photo(peer, filename); }
