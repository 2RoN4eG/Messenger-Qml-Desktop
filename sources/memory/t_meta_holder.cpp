...
void t_meta_holder::_do_unregister_out_of_range(const t_fs& fs) {
    ...
    const auto since = get_out_of_capacity_iterator(sorted, _capacity_range._minimum, sorted.begin(), cached_size);
    if (since == sorted.end()) {
        return;
    }

    if (get_out_of_capacity_iterator(sorted, _capacity_range._maximum, since, cached_size) == sorted.end()) {
        return;
    }
