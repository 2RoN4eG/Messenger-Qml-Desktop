#include "t_memory_meta_holder.h"
...
t_memory_meta_holder::t_memory_meta_holder(t_fs_size minimum_capacity, t_fs_size maximum_capacity, t_meta_holder_cache&& cache)
...
bool t_memory_meta_holder::does_exist(const t_fs_path& path) const {
    return _does_exist(path);
}

// const time (O(1))
void t_memory_meta_holder::do_register(const t_fs_path& path) {
    _do_register(path);
}

// const time (O(1))
void t_memory_meta_holder::do_unregister(const t_fs_path& path) {
    _do_unregister(path);
}

// Complexity does not matter (will be run separately in different thread)
void t_memory_meta_holder::do_rotate(const t_fs& fs) {
...
bool t_memory_meta_holder::_does_exist(const t_fs_path &path) const {
    return std::ranges::find(_cache, path) != _cache.end();
}

void t_memory_meta_holder::_do_register(const t_fs_path &path) {
    _cache.emplace(path);
}

// const time (O(1))
void t_memory_meta_holder::_do_unregister(const t_fs_path& path) {
...
// Complexity is O(N)
void t_memory_meta_holder::_do_unregister_out_of_range(const t_fs& fs) {
