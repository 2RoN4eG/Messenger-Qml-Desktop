...
protected:
    ...
    friend std::ostream& operator<<(std::ostream& stream, const t_value_holder<t_value>& holder)
    {
        stream << holder.value();
        return stream;
    }

    friend std::filesystem::path operator/(const std::filesystem::path& path, const t_value_holder<t_value>& holder)
    {
...
public:
    ...
    const t_value_holder<t_value> get_value_and_generate_next() { return _value ++; }

    const t_value_holder<t_value> get_value_and_generate_previous() { return _value --; }
