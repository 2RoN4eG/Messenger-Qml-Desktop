...
bool t_image_fs_worker::does_image_exist_on_drive(const t_fs_path& path) {
    if (path.empty()) {
...
t_qt_avatar t_image_fs_worker::read_image_from_drive(const t_fs_path& path) {
    if (path.empty()) {
...
void t_image_fs_worker::write_image_to_drive(const t_qt_avatar& avatar, const t_fs_path& path) {
    if (path.empty()) {
